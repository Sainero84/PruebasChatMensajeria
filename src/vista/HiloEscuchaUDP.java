...
public class HiloEscuchaUDP extends Thread {
	...
	private boolean clavePublicaRemotaLista = false;
	private boolean claveAESLista = false;
	...
	public void run() {
	    try {
	        byte[] buffer = new byte[4096]; // Tamaño aumentado
	        ...
	        while (!socket.isClosed()) {
	            ...
	            if (mensaje.startsWith("CHAT_PUBLICKEY")) {
	                ...
	                clavePublicaRemotaLista = true; // FLAG
	                ...
	                // Enviar AES cifrada con la clave pública remota
	                ...
	                socket.send(new DatagramPacket(msgAES.getBytes(StandardCharsets.UTF_8), 
	                                               msgAES.getBytes(StandardCharsets.UTF_8).length,
	                                               paqueteUDP.getAddress(),
	                                               paqueteUDP.getPort()));
	            ...
	            if (mensaje.startsWith("CHAT_KEY")) {
	                ...
	                claveAESLista = true; // FLAG
	                mostrarMensaje(">> Sistema: Clave AES establecida. Chat seguro activo.");
	                continue;
	            }

	            // Esperar hasta que se establezca la AES y clave pública
	            if (!clavePublicaRemotaLista || !claveAESLista) {
	                Thread.sleep(50); // Espera corta antes de procesar mensajes
	                continue;
	            }
